...
void menu()
{
    ...
    string expresion;
    FunctionParser fparser;
    double xi, xm, xra, xr, fxra, fpxr, fhxr, erpa, vals[] = {0}, h = 0.0000000000001;
    int i = 1;
    ...
    {
        system("cls");
        cout << "M\242todo Newton-Raphson" << endl << endl;
        ...
        switch(op)
        {
            ...
            case '2':
                ...
                if(expresion.size() != 0)
                {
                    ...
                    cout << "x inicial= "; cin >> xra;
                    
                    while(erpa > 0.0005)
                    {
                        vals[0] = xra; fxra = fparser.Eval(vals);
                        vals[0] = xra + h;
                        fhxr = fparser.Eval(vals);
                        fpxr = (fhxr-fxra)/h;//definición límite de la derivada
                        xr = xra - fxra/fpxr;                        
                        ...
                        cout << "Xi: " << " " << setprecision(p) << xra << endl;        
                        cout << "X(i+1): " << " " << setprecision(p) << xr << endl;                                
                        cout << "F(Xi): " << " " << setprecision(p) << fxra << endl;        
                        cout << "F'(Xi): " << " " << setprecision(p) << fpxr << endl;                                
...
int main()
{
    portada("Método Newton-Raphson", "3");
    menu();  
    return 0;
}
